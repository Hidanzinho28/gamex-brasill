(() => {
  ...
  .pix-top{position:static;background:transparent;backdrop-filter:none;border:0}
  .pix-copy-row{display:grid;grid-template-columns:minmax(0,1fr)}
  .pix-code{display:block;max-width:100%;user-select:all;min-height:44px}
  .pix-copy{min-height:52px;padding:14px 18px!important;font:800 15px Inter,sans-serif;background:linear-gradient(110deg,#ffad55,#ff791b);box-shadow:0 8px 26px rgba(245,116,26,.25);transition:transform .18s,box-shadow .18s,background .18s;touch-action:manipulation}
  .pix-copy:active{transform:scale(.98)}.pix-copy.is-copied{background:#94e7b3;color:#092716}.pix-copy:disabled{opacity:.45;cursor:not-allowed}
  .pix-copy-feedback{min-height:32px;margin:10px 0 0;color:#dfcab9;font-size:12px;line-height:1.5;text-align:center}
  @media(prefers-reduced-motion:reduce){.pix-modal,.pix-modal *,.pix-modal *::before{animation:none!important;transition:none!important;scroll-behavior:auto!important}}
  ...
  let pollTimer, previousFocus, savedOverflow;
  ...
  function displayPayment(data) {
    ...
    const copy = modal.querySelector('#copy-live-pix');
    const code = modal.querySelector('#pix-live-code');
    const feedback = modal.querySelector('#pix-copy-feedback');
    const value = String(payment.copyPaste || '').trim();
    copy.disabled = !value;
    if (!value) feedback.textContent = 'Código PIX indisponível. Feche esta janela e tente novamente.';
    let feedbackTimer;
    copy.onclick = async () => {
      if (!value) return;
      let copied = false;
      ...
      } catch (_) { /* Try the compatible copy method below. */ }
      if (!copied) {
        const input = document.createElement('textarea');
        input.value = value;
        input.readOnly = true;
        input.style.cssText = 'position:fixed;top:0;left:0;opacity:0;font-size:16px';
        modal.querySelector('.pix-dialog').appendChild(input);
        try {
          input.focus(); input.select(); input.setSelectionRange(0, value.length);
          copied = document.execCommand('copy') === true;
        } catch (_) { copied = false; }
        finally { input.remove(); copy.focus(); }
      }
      clearTimeout(feedbackTimer);
      copy.classList.toggle('is-copied', copied);
      copy.textContent = copied ? '✓ Código copiado!' : 'Tentar copiar novamente';
      feedback.textContent = copied ? 'Agora abra seu banco e cole em PIX Copia e Cola.' : 'Não foi possível copiar automaticamente. O código está selecionado: mantenha pressionado ou use Ctrl+C (⌘C no Mac).';
      if (!copied) {
        const range = document.createRange(); range.selectNodeContents(code);
        const selection = window.getSelection(); selection.removeAllRanges(); selection.addRange(range); code.focus();
      } else {
        feedbackTimer = setTimeout(() => { copy.textContent = 'Copiar código PIX'; copy.classList.remove('is-copied'); }, 3500);
      }
    };
  ...
  let saved = null;
  try { saved = JSON.parse(localStorage.getItem('gamex-active-payment') || 'null'); } catch (_) { localStorage.removeItem('gamex-active-payment'); }
